...
OBV_PIVOTS   = 3           # number of swing points to compare for divergence
...
async def scan_symbol(client: httpx.AsyncClient, symbol: str) -> dict | None:
    try:
        ...
        df_1h, df_4h, funding = await asyncio.gather(
            get_klines(client, symbol, "1h", 120),
            get_klines(client, symbol, "4h", 120),
...
def format_no_results() -> str:
    ...
    return (
        f"🔍 *Short Trade Screener* — {now}\n\n"
        f"No candidates found with score ≥ {MIN_SCORE}/{MAX_SCORE} this scan.\n"
        f"_Next scan in 4 hours._"
    )
...
async def run_scan():
    ...
    log.info(f"Scan complete. {len(results)} candidate(s) found.")

    if results:
        msg = format_alert(results)
    else:
        msg = format_no_results()
