...
PULLBACK_MIN_SWING   = 0.15  # 15% minimum swing (high - low) / low over last 50 4H candles
...
async def scan_symbol(client: httpx.AsyncClient, symbol: str) -> dict | None:
    try:
        ...
        # Hard gate — must pass pullback filter before scoring signals
        pullback = check_pullback(df_4h)
        ...
        fund = {
            "signal":   funding > FUNDING_THRESH,  # elevated — scores points
            "positive": funding > 0,               # any positive rate
            "negative": funding < 0,               # actively working against short
            "rate":     funding,
        }
...
def format_alert(results: list[dict]) -> str:
    ...
    for r in sorted(results, key=lambda x: x["score"], reverse=True):
        ...
        rate_pct = r["funding"]["rate"] * 100
        if r["funding"]["signal"]:
            fund_line   = "✅"
            fund_detail = f" ({rate_pct:.4f}% — longs paying shorts ✓)"
        elif r["funding"]["positive"]:
            fund_line   = "➖"
            fund_detail = f" ({rate_pct:.4f}% — mildly positive, not a strong signal)"
        else:
            fund_line   = "❌"
            fund_detail = f" ({rate_pct:.4f}% — negative, shorts paying longs)"

        bb_detail  = f" ({r['bb']['gap_pct']:+.2f}% from upper band)"
        pb = r["pullback"]
        pullback_line = f"📐 Swing: *{pb['swing_pct']}%* (high `{pb['swing_high']}` / low `{pb['swing_low']}`) · Retraced *{pb['retrace_pct']}%* from high"

        # Conditional 1H RSI line — only shown when funding is unfavourable and RSI 4H fired
        rsi_1h_line = ""
        if r.get("rsi_1h") is not None:
            v = r["rsi_1h"]["value"]
            flag = "⚠️" if r["rsi_1h"]["above_85"] else "ℹ️"
            status = "above 85 — strong short momentum on 1H" if r["rsi_1h"]["above_85"] else "below 85"
            rsi_1h_line = f"{flag} RSI 1H: *{v}* ({status}) — funding is negative"
        ...
        signal_lines = [
            ...
            f"{fund_line} Funding rate favours shorts{fund_detail}  [w:2]",
        ]
        if rsi_1h_line:
            signal_lines.append(rsi_1h_line)
